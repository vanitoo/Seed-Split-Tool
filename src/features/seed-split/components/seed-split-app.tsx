...
export function SeedSplitApp() {
  ...
  const recoveryShares = useMemo(() => recoveryInput.split(/\n\s*\n/u).map((value) => value.trim()).filter(Boolean), [recoveryInput]);
  ...
  async function createShares(): Promise<void> {
    ...
    } catch (error) { setStatus(error instanceof Error ? error.message : "Неизвестная ошибка"); }
  ...
  async function restore(input = recoveryShares): Promise<void> {
    ...
    } catch (error) { setRecovered(""); setStatus(error instanceof Error ? error.message : "Неизвестная ошибка"); }
    finally { setBusy(false); }
  }

  function clearAll(): void {
    setSecret(""); setShares([]); setRecoveryInput(""); setRecovered(""); setStatus(""); setVerified(false); setVisible(false);
  }
  ...
  return <main className="shell">
    ...
    <section className="warning"><strong>Важно:</strong> формат <code>SST1</code> является собственным форматом приложения, а не BIP39 или SLIP-39. Для восстановления сохраните программу вместе с частями.</section>
    ...
    {mode === "split" ? <section className="workspace">
      <div className="panel input-panel">
        ...
        <button className="primary" disabled={busy || !secret.trim()} onClick={createShares}>{busy ? "Обработка…" : `Создать ${total} частей`}</button>
      </div>
      ...
      <textarea value={recoveryInput} onChange={(event) => setRecoveryInput(event.target.value)} placeholder={"SST1-…\n\nSST1-…"} spellCheck={false} />
      <div className="meta"><span>Распознано частей: {recoveryShares.length}</span></div>
      <button className="primary" disabled={busy || recoveryShares.length === 0} onClick={() => restore()}>{busy ? "Проверка…" : "Восстановить секрет"}</button>
